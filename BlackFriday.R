...
bf<-read.csv("C:/Users/Pavan/Documents/NEU/SEM1-Winter/ALY6015-Intermediate Analytics/BlackFridaySales/BlackFriday.csv",stringsAsFactors=FALSE)
...
#Age Analysis
temp <- data.table(black_friday[c('User_ID','Age','Purchase','Product_ID')])
...
ggplot(black_friday, aes(y = Purchase)) +
...
ggplot(black_friday, aes(Occupation)) + geom_bar(fill= 'dark blue') + theme_classic()
...
Revenue <- black_friday %>% group_by(Product_ID) %>% summarise(revenue = sum(Purchase)) %>% arrange(desc(revenue))
...
ggplot(black_friday, aes(Purchase)) + 
